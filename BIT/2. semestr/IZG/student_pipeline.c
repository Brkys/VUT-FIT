...
void const* gpu_computeVertexAttributeDataPointer(
  ...
  (void)gl_VertexID;
  //Vzorec na wiki
  int *r = (int *)((head->stride * gl_VertexID) + head->buffer + head->offset);
  return r;
}
...
void gpu_runVertexPuller(
  ...
  (void)vertexShaderInvocation;
  int cntr =0;
  while(cntr < MAX_ATTRIBUTES)
  {
    if(&(puller->heads[cntr]) == NULL|| puller->heads[cntr].enabled == NULL)
      output->attributes[cntr] = NULL;
    else
      output->attributes[cntr] = gpu_computeVertexAttributeDataPointer(&(puller->heads[cntr]), gpu_computeGLVertexID(puller->indices, vertexShaderInvocation));
    cntr++;
  }
}
...
void gpu_runPrimitiveAssembly(
  ...
  size_t bvs = baseVertexShaderInvocation;
  GPUVertexShaderInput in;
  GPUVertexPullerOutput out;
  int cntr = 0;
  while(cntr < nofPrimitiveVertices)
  {
    in.gl_VertexID = gpu_computeGLVertexID(puller->indices, bvs + cntr);
    gpu_runVertexPuller(&out, puller, bvs + cntr);
    in.attributes = &out;
    vertexShader(&primitive->vertices[cntr], &in, gpu);
    primitive->nofUsedVertices += cntr;
    cntr++;
  }
}
...
void gpu_computeScreenSpaceBarycentrics(
  ...
  (void)vertices;
  (void)lines;
  float x = pixelCenter->data[0];
  float y = pixelCenter->data[1];
  float x1 = vertices[0].data[0], x2 = vertices[1].data[0], x3 = vertices[2].data[0];
  float y1 = vertices[0].data[1], y2 = vertices[1]. data[1], y3 = vertices[2].data[1];
  //Podle wiki vzorec na barycentrickou rovnici
  coords->data[0] = ((((y2-y3)*(x-x3)) + ((x3-x2)*(y-y3))) / (((y2-y3)*(x1 - x3)) + ((x3-x2)*(y1-y3))));
  coords->data[1] = ((((y3-y1)*(x-x3)) + ((x1-x3)*(y-y3))) / (((y2-y3)*(x1 - x3)) + ((x3-x2)*(y1-y3))));
